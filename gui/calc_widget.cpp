...
CalcWidget::CalcWidget(const Arguments& args): outStream(*args.outputStream),
        inputBoxGrid(new QHBoxLayout), nBox(new QSpinBox), lBox(new QSpinBox),
        pBox(new QSpinBox), testCheckBox(new newQCheckBox("Run &tests only")), 
    ...
    addWidget(inputBoxGrid);
    addWidget(testCheckBox);
    addWidget(goButton);
